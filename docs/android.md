...
Status: updated through the Stage 13 provider foundation, 20 September 2026. Sources are in `Unity_Connect_Android/app/src/main/java/com/unity/connect/android/`. See [PROGRESS.md](../PROGRESS.md) for continuation.
...
The UI provides pairing code comparison, connection status/address, reconnect/forget, a combined device-permission request, settings shortcuts for media/DND access, Windows media state/controls, current local Bluetooth audio output, companion-rule control and clipboard opt-in/manual send. Theme palettes are hard-coded light/dark schemes rather than dynamic system colors. Device access is requested as one bundle; missing optional phone-state permission continues to show the generic access action.
...
`BluetoothAudioMonitor` obtains the public A2DP profile proxy and reacts to profile connection broadcasts. With the existing nearby-device permission it reads the first connected output, removes control characters, limits the display label to 80 characters and publishes it only to local Android UI state. It deliberately has no connect/disconnect method and never exposes an address. Direct release and cross-device state remain gated by [ADR-005](decisions/ADR-005-headphone-handoff.md).
