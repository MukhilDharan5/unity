...
The UI provides pairing code comparison, connection status/address, reconnect/forget, a combined device-permission request, settings shortcuts for media/DND access, Windows media state/controls, companion-rule control and clipboard opt-in/manual send. Theme palettes are hard-coded light/dark schemes rather than dynamic system colors. Device access is requested as one bundle; missing optional phone-state permission continues to show the generic access action.
