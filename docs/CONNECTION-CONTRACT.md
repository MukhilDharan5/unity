...
Windows starts a remembered-device reconnect loop at startup and whenever either route is missing. It tries the saved Wi-Fi endpoint and bounded mDNS-discovered endpoints for Wi-Fi, and independently restores BLE, with capped pauses between rounds. A discovered endpoint replaces the saved endpoint only after authentication as the remembered phone. First pairing has a two-minute overall deadline and offers automatic LAN, manual LAN and BLE. Android starts both listeners automatically for a trusted laptop and when the user taps **Start pairing** for a new laptop; unpaired listeners and approval expire after two minutes.

Both peers may hold authenticated BLE and Wi-Fi sessions simultaneously. They route application messages through one deterministic active session, preferring Wi-Fi. If Wi-Fi closes, later messages use authenticated BLE and Android sends the latest snapshot over it. Windows preserves displayed phone state while either route remains connected and restores the missing route in the background. An ambiguous failed command is never replayed on the other route.
