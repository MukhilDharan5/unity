...
When Android reports a connected audio output, the flyout and full window expose **Move to laptop**. The action sends the authenticated `headphone_handoff` command and opens Windows Bluetooth settings. `HeadphoneHandoffAssistant` can optionally find `adb.exe` through `UNITY_CONNECT_ADB`, packaged/common Android SDK paths, the scrcpy directory or `PATH`. It proceeds only when exactly one authorized device is listed, and then opens Android's public Bluetooth settings action. It does not issue per-device shell disconnect commands, carry companion messages or run when the phone reports direct public release support.
