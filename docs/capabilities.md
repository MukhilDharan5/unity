...
| Audio streaming | Laptop -> phone PCM prototype over a separate encrypted Wi-Fi socket | Explicit **Play on phone** action, authenticated companion session, reachable LAN, current mono/stereo Windows output | DRM/call audio excluded; no phone capture, codec, route migration, or measured latency/battery behavior |
