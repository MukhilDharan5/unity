...
| 3 — LAN reliability (dual-route MVP implemented, validation deferred) | Automatic discovery plus concurrent authenticated BLE and Wi-Fi sessions; Wi-Fi primary, BLE warm fallback, missing-route recovery, snapshots and bounded liveness | No packet bonding or command replay; repeated sleep/network-change and dual-radio power tests remain |
