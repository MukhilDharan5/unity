...
| 13 — Headphone handoff (provider foundation complete; decision pending) | Android now observes A2DP output locally; ADR-005 records public API 37 association, assisted fallback and optional enhanced routes | No cross-device field/command or disconnect action until the control path is approved |
