...
The resizable desktop window provides Phone and Connection pages for the same existing controls and state as the flyout. Both views share one phone session and follow the system light/dark theme. The dashboard follows a Windows 11 Settings layout with a labeled navigation pane, battery/network/connection summaries, grouped setting rows, layered neutral surfaces, and the user's Windows accent color. Pairing and six-digit trust confirmation run smoothly inside the Connection page rather than opening another app window. Its native title bar and tray menu also follow the system theme. On supported integrated displays, an opt-in switch can use the phone's filtered light reading to adjust laptop brightness for the current app session.
...
The Windows UI follows the system light/dark preference and accent color. Its full app uses a Windows 11 Settings-style navigation pane, layered Fluent cards, modern buttons/toggles/sliders, rounded DWM corners and the native system backdrop where supported. The compact flyout uses the same palette and interaction states. The media player remains visible while playing, paused, idle, or disconnected; unavailable controls stay disabled. The Windows dashboard/flyout and Android app keep every implemented MVP capability visible, with clear idle, disconnected, unsupported, or permission-needed states instead of removing its UI. Escape, clicking outside, and Alt+F4 dismiss the flyout while keeping the tray app running. Use Exit to release it. Media buttons have keyboard focus, tooltips, and accessible labels. Effective DND and sound remain truthful phone-state labels; a separate **Companion DND** switch controls only the Android app's own automatic rule. The clipboard switch is off by default and handles new plain text only, with no history.
